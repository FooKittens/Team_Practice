...
namespace Teamcollab.Engine.Helpers
{
  public class IsoBatch : SpriteBatch
  {
    ...
    //    Matrix.CreateScale((float)Math.Sqrt(2f) / 2.0f, (float)Math.Sqrt(2f) / 2.0f, 1);
    ...
    {
      position = WorldManager.TransformIsometric(position);
      position = WorldManager.GetTileScreenPosition(position);
